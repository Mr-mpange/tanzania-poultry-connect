...
import { ArrowLeft, Camera, Loader2, Save, User, ShieldCheck, ShieldAlert, ShieldX, Upload, FileText, X } from "lucide-react";
...
export default function ProfileSettings() {
  ...
  useEffect(() => { if (user) fetchKycDocs(); }, [user]);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (!user) return;
    setSaving(true);
    const { error } = await supabase.from("profiles").update({ full_name: fullName, phone, location, avatar_url: avatarUrl || null }).eq("user_id", user.id);
    setSaving(false);
  ...
  const handleDocUpload = async (docType: string, e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Reset profile kyc_status to pending if it was rejected
              ...
              return (
                <div key={dt.value} className="bg-card border border-border rounded-xl p-4 flex items-center justify-between gap-3">
                  ...
                  <div className="flex-shrink-0">
                    <input
                      ref={el => { docFileRefs.current[dt.value] = el; }}
                      type="file" accept="image/*,.pdf" className="hidden"
                      onChange={e => handleDocUpload(dt.value, e)}
                    />
                    <button
                      onClick={() => docFileRefs.current[dt.value]?.click()}
